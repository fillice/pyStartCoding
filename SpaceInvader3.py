...
class Player:  # 주인공 관련
    def __init__(self):  # 초기화
        #self.x = 320
        #self.y = 570
        self.x = screen.get_width()/2-playerImg.get_width()/2
        self.y = screen.get_height()-(playerImg.get_width()/2+playerImg.get_height()/2)
        self.dir = 0
    ...
    def turn(self):
        if pressedKeys[K_UP] and self.dir < 90:
            self.dir += 1
        if pressedKeys[K_DOWN] and self.dir > -90:
            self.dir -= 1
    ...
    def fire(self):  # 미사일 리스트에 새 미사일 추가
        ...
        shots += 1
        missiles.append(
            Missile(self.x+playerImg.get_width()/2,
                    self.y+playerImg.get_height()/2, self.dir))
    ...
    def draw(self):  # 화면에 그리기
        rotated = pygame.transform.rotate(playerImg, self.dir)
        screen.blit(rotated, (self.x+playerImg.get_width()/2-rotated.get_width()/2,
                              self.y+playerImg.get_height()/2-rotated.get_height()/2))
...
class Missile:
    ...
    def draw(self):
        ...
        pygame.draw.line(screen, (255, 0, 0), (self.x, self.y),
                         (self.x-self.dx*5, self.y-self.dy*5 + 8), 5)
...
while 1:
    ...
    for enemy in enemies:
        if player.hitBy(enemy):
            ...
            # 여러 정보들 텍스트 띄우기
            screen.blit(font.render("Total Shots: "+str(shots),
                                    True, (255, 255, 255)), (180, 320))
            screen.blit(font.render("Score: "+str(score),
                                    True, (255, 255, 255)), (180, 370))
            screen.blit(font.render("On Target: "+str(hits),
                                    True, (255, 255, 255)), (350, 320))
            screen.blit(font.render("Missed: "+str(misses),
                                    True, (255, 255, 255)), (350, 370))
            if shots == 0:  # 0나눔 에러 방지용
                screen.blit(font.render("Accuracy: --", True,
                                        (255, 255, 255)), (350, 420))
            else:
